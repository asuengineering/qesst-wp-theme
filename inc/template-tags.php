	...
	function understrap_posted_on() {
		...
		$byline      = apply_filters(
			'understrap_posted_by', sprintf(
				'<span class="byline"><i class="far fa-edit"></i>%1$s<span class="author vcard"><a class="url fn n" href="%2$s"> %3$s</a></span></span>',
	...
	function understrap_entry_footer() {
		...
		if ( 'post' === get_post_type() ) {
			...
			if ( $categories_list && understrap_categorized_blog() ) {
				...
				printf( '<span class="cat-links"><i class="fas fa-folder-open"></i>' . esc_html__( 'Posted in %s', 'understrap' ) . '</span>', $categories_list ); // WPCS: XSS OK.
			}
			/* translators: used between list items, there is a space after the comma */
			$tags_list = get_the_tag_list( '', esc_html__( ', ', 'understrap' ) );
			if ( $tags_list ) {
				/* translators: %s: Tags of current post */
				printf( '<span class="tags-links"><i class="fas fa-tag"></i>' . esc_html__( 'Tagged %s', 'understrap' ) . '</span>', $tags_list ); // WPCS: XSS OK.
			}
		}
